...
step_size = 0.1
boundary = 1.0
boundary_xmin, boundary_xmax = -0.5, 0.5
boundary_ymin, boundary_ymax = -0.5, 0.5
...
positions = [(x, y)]
potential_energies_tilde = [potential(x,y)/degree_of_overlap(x,y)]
...
for _ in range(n_steps):
    ...
    potential_energies_tilde.append(V_current/degree_of_overlap(x,y))
...
potential_energies_tilde = np.array(potential_energies_tilde)
DOOs = np.array(DOOs)
print("mean 1/DOO = %.3f, mean potential energy tilde = %.3f"%(np.mean(1/np.array(DOOs)), np.mean(np.array(potential_energies_tilde))))
